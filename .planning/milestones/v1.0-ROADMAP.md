...
**Phases:** 1-3 (Fundação e Acesso; Lançamentos Manuais; Ditado por Voz — Core)
**Total Plans:** 9 | **Requisitos validados:** 10/31

## Overview

Primeira versão utilizável do "Meu Bolso": fundação sólida (app Flutter multi-plataforma + login no Supabase), lançamentos de despesas manuais sincronizados via realtime e o Core — registrar gastos **falando**, com a IA preenchendo e o usuário confirmando com um toque. Publicado no GitHub Pages.
...
## Milestone Summary (v1.0 MVP)

**Key Decisions:**

- Stack: Flutter + Supabase (auth + Postgres + realtime) + Gemini (free tier) — validada em produção
- Deploy no GitHub Pages com URLs hash-based (web público; Android/iOS pendente de toolchain)
- Modelo de captura por voz: `gemini-3.5-flash-lite` (free tier) — após testes reais; `gemini-2.5-flash` descontinuado para chaves novas, 3.x em 503 sob carga; retry 3x no app como defesa
- Lançamentos como inteiro em centavos (BRL); categorias/formas de pagamento fixas em PT-BR
- Dados sensíveis nunca versionados (`.env`, seed com senhas, SQL local fora do git)

**Issues Resolved:**

- 503/429 intermitentes do Gemini free tier → retry com backoff (200ms/800ms) no `GeminiDitadoRepository`
- Erro de categoria incorreta → prompt reforçado (lista fixa, fallback "Outros"/"Outro", mapeamentos explícitos)
- Gravação fake instantânea nos testes → `ditadoRelogioProvider` para gate <500ms

**Issues Deferred:**

- Validação em dispositivo Android/iOS (toolchain não instalada) — PLAT-02 parcial
- `01-01-SUMMARY.md` não foi criado na Fase 1 (registro histórico incompleto)

**Technical Debt Incurred:**

- `lancamento_form_screen.dart` e `confirmacao_ditado_screen.dart` pré-preenchem os 6 campos de forma duplicada (fase 4 pode extrair um widget compartilhado)
- Gemini `data`/`vencimento` preenchidos pela IA quando não citados (defensivo: só salvar com confirmação)

---
*Para o estado atual do projeto, ver `.planning/ROADMAP.md`.*
